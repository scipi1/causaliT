...
class DAGMask(nn.Module):
    ...
    """
    def __init__(self, n_heads, query_seq_len, key_seq_len):
        super(DAGMask, self).__init__()
        if n_heads > 1:
            self.phi = nn.Parameter(torch.zeros(n_heads, query_seq_len, key_seq_len))
        else:
            self.phi = nn.Parameter(torch.zeros(query_seq_len, key_seq_len))
