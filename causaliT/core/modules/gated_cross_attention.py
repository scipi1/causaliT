"""
GatedCrossAttention: structure-gated cross attention (Hard-Concrete L0 gate).

Design
======
The edge score is a single **structure gate**::

    A_ij = z_ij

* ``z_ij`` - **structure gate**.  A Hard-Concrete L0 gate (Louizos et al.,
  ICLR 2018) computed from a *structural* query/key pair
  ``z_ij = HardConcrete(<q^s_i, k^s_j> * scale)``.  Stochastic-binary in
...
The reconstruction magnitude is carried entirely by the value stream
(``W_V`` / FFN / head), so the gate stays a clean, thresholdable {0,1}
adjacency.  The former multiplicative reconstruction-gain stream
(``A = z * g``) has been REMOVED: it conflated structure with magnitude and
was unsafe.
...
functions of the **gate** only, so they act purely on structure.  Evaluation
should threshold ``last_p_edge_on`` (the gate posterior edge probability) to
obtain the recovered adjacency.
...
structure gate ``A = z``, whose *sampled* value is not directly thresholdable.
Because ``out`` (first element) is already computed from ``A`` internally and
**no downstream calculation consumes the returned ``attn`` for
reconstruction** - the training loss reads ``out``, the residuals, the
``score_tensor_for_sparsity`` attribute and ``aux``; only the
evaluation/DAG-extraction path reads the returned ``attn`` - GCA reuses this
...
``query`` / ``key`` are the *structural* (gate) projections, expected
pre-projected with a single structural head (3-D: ``(B, L, E)`` /
``(B, S, E)``) - i.e. ``shared_dag_across_heads=True``.  ``value`` may be
3-D ``(B, S, d)`` or 4-D ``(B, S, H, d)`` (multiple value heads sharing the
single gate score).
"""
...
class GatedCrossAttention(nn.Module):
    """Structure-gated cross attention (Hard-Concrete L0 gate)."""
    ...
    def __init__(
        ...
        # (up to in-degree query_fanin_scale) stay clearly ON.
    ...
    def structure_posterior(
        ...
        Structure only: no Hard-Concrete noise, no value aggregation and no
        ``last_*`` diagnostics write, so the transitive-correction trigger is
    ...
    def forward(
        ...
        # normalize_query:    q_hat = q/||q||  and  log_alpha = <q_hat, k^s> * sqrt(fanin)
        ...
        if oracle:
            ...
            # z_ij = hard_mask_ij (true topology, held constant).  Used to
            ...
            # (0 = residual floor, 1 = uniform mixing) so the value
            # reconstruction capacity is measurable independently of an
            # untrained structure.  Applies in BOTH train and eval
            # (deterministic).
        ...
        # ---- Final attention weight: the structure gate IS the weight ----
        A = z                                            # (B, L, S)
        ...
        # ``A`` here is the TRUE applied weight (the gate, masked, dropped).
        ...
        # ---- Entropy (over the gate weights, for logging) ----------------
        entropy = None
        ...
        # (masked P(z>0), shape (B, L, S)) instead of the applied weight A=z.
