"""
Predictor for SingleCausalForecaster single-decoder architecture.
"""
...
class SingleCausalPredictor(BasePredictor):
    """
    Predictor for SingleCausalForecaster single-decoder models.
    
    Handles:
    - SingleCausalForecaster with single decoder (S → X)
    - Three-input data format (S, X, Y) - but only uses S and X
    - Single output (pred_x)
    ...
    Note:
        The blanking of X values is handled internally by SingleCausalForecaster.forward().
        It blanks X[:, :, val_idx] before passing to the decoder.
    """
    
    def _load_model(self) -> SingleCausalForecaster:
        """
        Load SingleCausalForecaster model from checkpoint.
        ...
        Returns:
            Loaded SingleCausalForecaster model with masks loaded if applicable
        """
        model = SingleCausalForecaster.load_from_checkpoint(self.checkpoint_path)
    ...
    def _load_hard_masks_for_model(self, model: SingleCausalForecaster):
        """
        ...
        Args:
            model: SingleCausalForecaster model to load masks into
        """
    ...
    def create_data_module(
        ...
        Note: Uses StageCausalDataModule since SingleCausalForecaster also expects
    ...
    def _forward(self, S: torch.Tensor, X: torch.Tensor, Y: torch.Tensor = None,
                 use_hard_masks: bool = None, **kwargs) -> Any:
        """
        Perform forward pass through SingleCausalForecaster model.
        
        Note:
            The SingleCausalForecaster.forward() method handles blanking internally:
        ...
        Args:
            ...
            use_hard_masks: Override for hard mask usage. If None (default), uses
                           the value from config (model.use_hard_masks).
                           Set to False to disable hard masks for ablation studies.
            **kwargs: Additional arguments
            
        Returns:
            Tuple of (pred_x, attention_weights, masks, entropies)
        """
        # SingleCausalForecaster.forward() handles the blanking of X values internally
    ...
    def _process_forward_output(self, output: Any) -> Dict[str, Any]:
        """
        Process SingleCausalForecaster model output.
    ...
    def predict(
        ...
        **kwargs
        ...
        """
        Run prediction on specified dataset for SingleCausalForecaster model.
        ...
        Returns:
            PredictionResult object containing:
                ...
                - outputs: pred_x (X predictions)
                - targets: X (X actual)
                - attention_weights: Dict with dec_cross, dec_self
                - metadata: Contains model info
        """
        ...
        for batch in tqdm(dataset):
            ...
            with torch.no_grad():
                output = self._forward(S, X, use_hard_masks=use_hard_masks, **kwargs)
        ...
        return PredictionResult(
            ...
            outputs=pred_x_array,     # pred_x (X predictions)
