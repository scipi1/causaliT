"""
Predictor for NoiseAwareCausalForecaster architecture.

Handles the noise-aware model which outputs (mu, log_var) instead of point predictions.
"""
...
class NoiseAwareCausalPredictor(BasePredictor):
    """
    Predictor for NoiseAwareCausalForecaster noise-aware models.
    
    Handles:
    - NoiseAwareCausalForecaster with Gaussian output (mu, log_var)
    - Three-input data format (S, X, Y) - but only uses S and X
    - Returns mean predictions along with uncertainty estimates
    - Automatic loading of hard masks if model was trained with them
    
    Returns predictions along with attention weights from the decoder:
    - Decoder: S → X (dec_cross, dec_self)
    
    Note:
        The blanking of X values is handled internally by NoiseAwareCausalForecaster.forward().
        It blanks X[:, :, val_idx] before passing to the decoder.
    """
    
    def _load_model(self) -> NoiseAwareCausalForecaster:
        """
        Load NoiseAwareCausalForecaster model from checkpoint.
        ...
        Returns:
            Loaded NoiseAwareCausalForecaster model with masks loaded if applicable
        """
        model = NoiseAwareCausalForecaster.load_from_checkpoint(self.checkpoint_path)
    ...
    def _load_hard_masks_for_model(self, model: NoiseAwareCausalForecaster):
        """
        ...
        Args:
            model: NoiseAwareCausalForecaster model to load masks into
        """
    ...
    def create_data_module(
        ...
        Note: Uses StageCausalDataModule since NoiseAwareCausalForecaster also expects
        (S, X, Y) format for compatibility, even though Y is ignored during training.
    ...
    def _forward(self, S: torch.Tensor, X: torch.Tensor, Y: torch.Tensor = None,
                 disable_hard_masks: bool = False, inject_noise: bool = False, **kwargs) -> Any:
        """
        Perform forward pass through NoiseAwareCausalForecaster model.
        
        Note:
            The NoiseAwareCausalForecaster.forward() method handles blanking internally:
            - X values at val_idx are blanked (set to 0.0)
        ...
        Args:
            ...
            Y: Target tensor (B x L_y x F) - ignored for NoiseAwareCausalForecaster
            disable_hard_masks: If True, disables hard masks even if model was trained with them.
                               Useful for ablation studies during inference. Default False.
            inject_noise: If True, inject ambient noise during forward pass. Default False
                         for inference (deterministic predictions).
            **kwargs: Additional arguments
            
        Returns:
            Tuple of (mu, log_var, attention_weights, masks, entropies)
        """
        # NoiseAwareCausalForecaster.forward() handles the blanking of X values internally
    ...
    def _process_forward_output(self, output: Any) -> Dict[str, Any]:
        """
        Process NoiseAwareCausalForecaster model output.
        
        Args:
            output: Tuple from model.forward():
                (mu, log_var, attention_weights, masks, entropies)
                where attention_weights = (dec_cross, dec_self)
        
        Returns:
            Dictionary with:
                - 'pred_x': X predictions (mu)
                - 'pred_log_var': Log variance predictions
                - 'pred_std': Standard deviation predictions
                - 'attention_weights': Dict with 'dec_cross', 'dec_self'
        """
        mu, log_var, attention_weights_tuple, masks, entropies = output
    ...
    def predict(
        ...
        **kwargs
        ...
        """
        Run prediction on specified dataset for NoiseAwareCausalForecaster model.
        
        This overrides the base predict() method to handle the three-input
        data format (S, X, Y) used by the dataloader, though Y is ignored.
        
        Note:
            The blanking of X values is handled internally by the model's
            forward method. X[:, :, val_idx] is set to 0.0 before being
        ...
        Args:
            ...
            disable_hard_masks: If True, disables hard masks even if model was trained with them.
            inject_noise: If True, inject ambient noise (stochastic predictions). Default False.
            **kwargs: Additional arguments passed to forward
            
        Returns:
            PredictionResult object containing:
                - inputs: S (source data)
                - outputs: pred_x (X predictions, i.e., mu)
                - targets: X (X actual)
                - attention_weights: Dict with dec_cross, dec_self
                - metadata: Contains model info, including pred_std for uncertainty
        """
        ...
        pred_x_list = []     # X predictions (mu)
        pred_std_list = []   # Prediction uncertainty (std)
        ...
        for batch in tqdm(dataset):
            ...
            pred_std_list.append(processed['pred_std'].cpu())
        ...
        # Create metadata (include uncertainty for analysis)
        metadata = {
            'model_type': self.config["model"]["model_object"],
            'dataset_label': dataset_label,
            'batch_size': self.config["training"]["batch_size"],
            'num_samples': len(s_array),
            'pred_std': pred_std_array,  # Include uncertainty in metadata
            'inject_noise': inject_noise,
        }
        
        return PredictionResult(
            inputs=s_array,           # S (source data)
            outputs=pred_x_array,     # pred_x (X predictions = mu)
            targets=x_array,          # X (X actual)
            attention_weights=attention_weights,
            metadata=metadata
        )
